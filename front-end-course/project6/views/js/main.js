...
var pizzaElementGenerator = function(i) {
    ...
    pizzaDescriptionContainer.style.width = "60%";
...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        for (var i = 0; i < randomPizzas.length; i++) {
            // no need for complex Dx calculate, just change the size of pizzas based on it size (small, medium and large)
...
function updatePositions() {
    ...
    for (var i = 0; i < 5; i++) {
        phase.push(Math.sin(latestKnownScrollY / 1250 + i) * 100);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    var width = window.innerWidth; // get the window current width
    ...
    var numberPizzas = Math.ceil(cols * rows); // generate the number of pizzas based on the width and height
    var movingPizzas = document.getElementById("movingPizzas1");
    for (var i = 0; i <= numberPizzas; i++) {
        var elem = document.createElement('img');
        elem.className = 'mover';
        elem.src = "images/pizza.png";
        elem.style.transform = 'translate3d(0, 0, 0) translate(0px)'; // added translate 3d for use GPU
        elem.height = "100";
        elem.width = "73";
        elem.style.height = elem.height + 'px';
        elem.style.width = elem.width + 'px';
        elem.basicLeft = (i % cols) * s; // add left position based on the column
